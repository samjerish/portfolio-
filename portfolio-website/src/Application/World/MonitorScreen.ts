...
export default class MonitorScreen extends EventEmitter {
    ...
    createLogoOverlay() {
        // Create a canvas to draw the logo texture
        ...
        // Background color (matching the bezel)
        ctx.fillStyle = '#b0acab';
        ctx.fillRect(0, 0, canvas.width, canvas.height);

        // Draw Globe Icon manually (approximate)
        ...
        ctx.stroke();
        ctx.beginPath();
        ctx.ellipse(64, 64, 16, 40, 0, 0, Math.PI * 2);
        ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(24, 64);
        ctx.lineTo(104, 64);
        ...
        // Draw Text "SAM JERISH D"
        ctx.fillStyle = '#333333';
        ctx.font = 'italic 900 48px Arial';
        ctx.fillText('SAM JERISH D', 120, 60);

        // Draw Text "sam jerish d inc"
        ctx.fillStyle = '#555555';
        ctx.font = 'italic 24px Arial';
        ctx.fillText('sam jerish d inc', 124, 95);
        ...
        // Create mesh
        const geometry = new THREE.PlaneGeometry(160, 40); // Size in 3D units
        ...
        // Offset to the bezel area
        mesh.position.x -= 400; // Left
        mesh.position.y -= 540; // Down
        mesh.position.z += 10;  // Forward
